...
RSpec.describe "Cancel Friend Request", type: :system, js: true do 
  ...
  describe "users#show page" do
    ...
    context "when clicking the cancel button on the users#show page" do
      it "successfully cancels the friend request on the users#show page using Turbo Streams" do
        new_user = create(:user)
        create(:friend_request, sender: user, receiver: new_user)
